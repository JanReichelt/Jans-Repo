...
/// set classes
  class Intersection{
        ...
        for (let elem in this.neighbors) {
          if (this.neighbors[elem]) {
            if (this.neighbors[elem].state == this.state) {
              ...
              console.log(this.neighbors[elem].group.members.length);
              ...
              // for (let i = 0; i < this.neighbors[elem].members.length; i++) {
              //   this.group.members.push(this.neighbors[elem].members[i]);
              // }
    ...
    }///////////ACHTUNG!! Beachte, dass anscheinend nur die members bei den aktuellen
    // gesetzten Steinen aktualisiert werden. Der erste Stein der Gruppe bekommt seine
    //this.members nicht aktualisiert, was spätestens bei den Freiheiten zu Problemen führt
    // Lösbar dadurch, dass this.group nur eine Referenz auf die globale groups.Gruppe ist?!
  }
  ...
  class Group{
    ...
    constructor(intersection){
       // "Size" ist nicht mehr nötig, da members.length
      this.members.push(intersection);

      // console.log("members erstellt:");
