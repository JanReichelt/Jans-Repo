...
function clickHandler(event) {
    ...
    if (g) {
        ...
        for (let i = 0; i < g.cols*g.rows; i++) {
            ...
            && mouseY >= g.grid[i].y) {
                console.log("mouse location found");
                if (g.grid[i].hidden == true){
                    console.log("check: field hidden true");
                    if (g.moveNr == 0) {
                        g.setMines(i);
                    }
                    g.grid[i].unveil();
...
class Game {
    ...
    setMines(index) {
        ...
        while(i <= this.mineNr) {
            ...
            if ((item.j*this.cols+item.i) !== index && item.state !== 'mine') {
                console.log("set mines");
                console.log(item);
                console.log(`index: ${index}`);
                console.log(item.j);
                console.log(this.cols);
                console.log(`index: ${index}; j: ${item.i*this.cols+item.j}`);
                console.log(`index: ${index}; j: ${item.i+this.cols*item.j}`);
                item.state = 'mine';
                i++;
            }
